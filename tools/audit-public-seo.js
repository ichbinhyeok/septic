...
return await (async () => {
  ...
  window.publicAudit = results.sort((a,b)=>a.path.localeCompare(b.path)); return JSON.stringify({count:results.length,issues:results.filter(p=>p.error||p.status!==200||!p.canonical||p.h1.length!==1)});
})()
