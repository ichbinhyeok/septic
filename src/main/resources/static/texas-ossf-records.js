(() => {
    ...
    function countyInstructionsUrl(path){if(!prepared||prepared.type!=="address"||!path.startsWith("/"))return path;const url=new URL(path,window.location.origin);url.searchParams.set("address",prepared.clue);return `${url.pathname}${url.search}${url.hash}`;}
